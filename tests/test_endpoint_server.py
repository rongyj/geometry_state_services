...
class StateEndpointHTTPRequestTestSuite(unittest.TestCase):
    ...
    def test_endpoint_request(self):
        ...
        endpoint_server = HTTPServer((socket.gethostbyname(socket.gethostname()), 8080), services.http.endpoint.StateEndpointHTTPRequestHandler)
        ...
        # Test GET
        print(urlopen("http://"+socket.gethostname()+":8080/?longitude=-77.036133&latitude=40.513799").read().decode())

        #Test POST
        post_data = parse.urlencode({"longitude":-77.036133,"latitude":40.513799}).encode()
        req = request.Request("http://"+socket.gethostname()+":8080", data = post_data)  # this will make the method "POST"
